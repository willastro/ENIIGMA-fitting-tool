...
def stat(DIR = os.getcwd()+'/', f_sig=5):
	...
	sga.stats_block(xdata0, new_tau, err_tau, fileout, filecl, home1, f_sig=f_sig)
	
	for f2 in glob.glob(home1+'Colum_*.csv'):
		os.remove(f2)
	for f3 in glob.glob(home1+'trans_*.csv'):
		os.remove(f3)
	for f4 in glob.glob(home1+'Colum_density_*.csv'):
		os.remove(f4)
