...
def min_max(xd, yd, emin, emax, pathdir):
	...
	Cmin0 = pd.read_csv(pathdir+'Confidence_limits_2nd.dat',sep='\s+', header=None, usecols=list(range(t0.shape[1]-1)), nrows=1)
	...
	custom_lines = [Line2D([0], [0], color='black', lw=1.5), Line2D([0], [0], color='limegreen', lw=1.5),Line2D([0], [0], color='red', lw=1.5,ls='--'), Line2D([0], [0], color='blue', lw=1.5, ls='--')] #Line2D([0], [0], color='grey', lw=1.5),
	plt.legend(custom_lines, ['Observation','Best fit', 'Lower confidence limit', 'Upper confidence limit'], loc='lower left', frameon=False, fontsize = 'small')
	plt.minorticks_on()
