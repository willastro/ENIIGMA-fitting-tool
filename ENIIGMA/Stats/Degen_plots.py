...
def merge_components_cd(dir = os.getcwd() + '/'):
	...
	for index in range(len(file_csv)):
		...
		try:
			...
			for f1 in glob.glob(pathdir+'Analytic*.dat'):
				os.remove(f1)
			for f2 in glob.glob(pathdir+'Colum_*.csv'):
				os.remove(f2)
			for f3 in glob.glob(pathdir+'trans_*.csv'):
				os.remove(f3)
			for f4 in glob.glob(pathdir+'Colum_density_*.csv'):
				os.remove(f4)
	...
	df = pd.concat(list(map(pd.read_csv, glob.glob(os.path.join('', "output_*.csv")))), sort='False') #sort = False because pandas warning
...
def hist_plot(dir = os.getcwd() + '/'):
	...
	for index,index1 in zip(r1,r2):
		...
		m = np.median(sp_v2)
		n, bins, patches = ax.hist(np.log(sp_v2/m),bins='fd', range=[-3,3], ls='solid', facecolor='darkred', alpha=1., histtype='bar', ec='black', label='$N_\mathrm{H_2O,\ tot}$')
		maxfreq = n.max()
		...
		new_header = header[index].replace('2','$_2$').replace('3','$_3$').replace('4','$_4$').replace('+','$^+$')#.replace('COO','$\mathrm{COO}^-$')
		plt.title('Molecule:'+' '+new_header+ '\n' +'3$\sigma$ C.I.'+':'+' '+str('{:.2f}'.format(q_avg))+'$_{'+str('{:.2f}'.format(q_min))+'}^{'+str('{:.2f}'.format(q_max))+'}$'+' '+'x'+'$10^{17}$'+'  '+'$\mathrm{cm^{-2}}$', position=(0.5, 0.75), fontsize=13, color='blue')
		if count1 % 2 != 0:
