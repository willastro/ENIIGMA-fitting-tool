...
def stats_block(ntt, new_tau, etau, fileout, filecl, pathdir, f_sig=5):
	"""
	...
	if w_values == 2:
		...
		try:
			...
			filename = pathdir+'Colum_density_*.csv'
		...
		for i1,j1,k1,l1,m1 in zip(list2, list2, list2, list2, list2):
			f1 = int1[i1]*dainp[:,1] + int2[j1]*dainp[:,3] + int3[k1]*dainp[:,5] + int4[l1]*dainp[:,7] + int5[m1]*dainp[:,9]
			
			p = (1./(len(ydata0)-1.-w_values))*np.sum(((ydata0-f1)/err_y)**2)
		
			if int1[i1] >= 0. and int2[j1] >= 0. and int3[k1] >= 0. and int4[l1] >= 0. and int5[m1] >= 0. and p >= 0. and p <= 1100.:
				fp = open('Confidence_limits_2nd.dat','a')
				fp.write('{0:f} {1:f} {2:f} {3:f} {4:f} {5:f}\n'.format(int1[i1],int2[j1], int3[k1], int4[l1], int5[m1], p))
				fp.close()
		...
		for i1,j1,k1,l1,m1,n1 in zip(list2, list2, list2, list2, list2, list2):
			f1 = int1[i1]*dainp[:,1] + int2[j1]*dainp[:,3] + int3[k1]*dainp[:,5] + int4[l1]*dainp[:,7] + int5[m1]*dainp[:,9] + int6[n1]*dainp[:,11]
			
			p = (1./(len(ydata0)-1.-w_values))*np.sum(((ydata0-f1)/err_y)**2)
		
			if int1[i1] >= 0. and int2[j1] >= 0. and int3[k1] >= 0. and int4[l1] >= 0. and int5[m1] >= 0. and int6[n1] >= 0. and p >= 0. and p <= 1100.:
				fp = open('Confidence_limits_2nd.dat','a')
				fp.write('{0:f} {1:f} {2:f} {3:f} {4:f} {5:f} {6:f}\n'.format(int1[i1],int2[j1], int3[k1], int4[l1], int5[m1], int6[n1], p))
				fp.close()
		...
		int5 = np.random.normal(c5, m, 1000)
		int6 = np.random.normal(c6, m, 1000)
		int7 = np.random.normal(c7, m, 1000)
		
		list2 = list(range(len(int1)))
		
		for i1,j1,k1,l1,m1,n1,o1 in zip(list2, list2, list2, list2, list2, list2, list2):
			f1 = int1[i1]*dainp[:,1] + int2[j1]*dainp[:,3] + int3[k1]*dainp[:,5] + int4[l1]*dainp[:,7] + int5[m1]*dainp[:,9] + int6[n1]*dainp[:,11] + int7[o1]*dainp[:,13]
			
			p = (1./(len(ydata0)-1.-w_values))*np.sum(((ydata0-f1)/err_y)**2)
		
			if int1[i1] >= 0. and int2[j1] >= 0. and int3[k1] >= 0. and int4[l1] >= 0. and int5[m1] >= 0. and int6[n1] >= 0. and int7[o1] >= 0. and p >= 0. and p <= 1058.:
				fp = open('Confidence_limits_2nd.dat','a')
				fp.write('{0:f} {1:f} {2:f} {3:f} {4:f} {5:f} {6:f} {7:f}\n'.format(int1[i1],int2[j1], int3[k1], int4[l1], int5[m1], int6[n1], int7[o1], p))
				fp.close()
		...
		c1,c2,c3,c4,c5,c6,c7,c8 = tcl[0][0], tcl[1][0], tcl[2][0], tcl[3][0], tcl[4][0], tcl[5][0], tcl[6][0], tcl[7][0]
		m = f_sig*np.std(err_y)
		int1 = np.random.normal(c1, m, 1000)
		int2 = np.random.normal(c2, m, 1000)
		int3 = np.random.normal(c3, m, 1000)
		...
		int6 = np.random.normal(c6, m, 1000)
		int7 = np.random.normal(c7, m, 1000)
		int8 = np.random.normal(c8, m, 1000)
		
		list2 = list(range(len(int1)))
		
		for i1,j1,k1,l1,m1,n1,o1,q1 in zip(list2, list2, list2, list2, list2, list2, list2,list2):
			f1 = int1[i1]*dainp[:,1] + int2[j1]*dainp[:,3] + int3[k1]*dainp[:,5] + int4[l1]*dainp[:,7] + int5[m1]*dainp[:,9] + int6[n1]*dainp[:,11] + int7[o1]*dainp[:,13] + int8[q1]*dainp[:,15]
			
			p = (1./(len(ydata0)-1.-w_values))*np.sum(((ydata0-f1)/err_y)**2)
		
			if int1[i1] >= 0. and int2[j1] >= 0. and int3[k1] >= 0. and int4[l1] >= 0. and int5[m1] >= 0. and int6[n1] >= 0. and int7[o1] >= 0. and int8[q1] >= 0. and p >= 0. and p <= 100.:
				fp = open('Confidence_limits_2nd.dat','a')
				fp.write('{0:f} {1:f} {2:f} {3:f} {4:f} {5:f} {6:f} {7:f} {8:f}\n'.format(int1[i1],int2[j1], int3[k1], int4[l1], int5[m1], int6[n1], int7[o1], int8[q1], p))
				fp.close()
