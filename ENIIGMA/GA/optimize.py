...
def ENIIGMA(od_file, xmin, xmax, list_sp, n_points=500, group_comb=5, skip = False, pathlib = None, factor1 = 2, factor2 = 2, dtype = 'Obs',St = 'False', freqs = 10, gen=100, ga_min = 0., ga_max = 0.5, mutp = 0.01, popsize = 50):
	"""
	...
	try:
		new_tau = interp1d(x_lam[ind1:ind2],tau[ind1:ind2], kind='cubic', bounds_error=False,fill_value=0.05)	#interpolate data
		new_tau = new_tau(ntt)
	
		new_etau = interp1d(x_lam[ind1:ind2],etau[ind1:ind2], kind='cubic', bounds_error=False,fill_value=0.05)	#interpolate data
		new_etau = new_etau(ntt)
	
	except ValueError:
		new_tau = interp1d(x_lam[ind1:ind2],tau[ind1:ind2], kind='linear', bounds_error=False,fill_value=0.05)	#interpolate data
		new_tau = new_tau(ntt)
	
		new_etau = interp1d(x_lam[ind1:ind2],etau[ind1:ind2], kind='linear', bounds_error=False,fill_value=0.05)	#interpolate data
		new_etau = new_etau(ntt)
	...
	if skip == False:
		...
		for index0 in range(i_var):
			...
			if bool(res) == True:
				...
				for index in range(i):
					...
					try:
						tau_lab = interp1d(x_mic[ind11:ind22],tau[ind11:ind22], kind='cubic', bounds_error=False,fill_value=0.05)
						tau_lab = abs(tau_lab(ntt))#+0.01
					except ValueError:
						tau_lab = interp1d(x_mic[ind11:ind22],tau[ind11:ind22], kind='linear', bounds_error=False,fill_value=0.05)
						tau_lab = abs(tau_lab(ntt))#+0.01
				...
				f = gam.gamod(new_tau,new_etau,home1,home2, Stats=St, freq_stat = freqs, gen=gen, ga_min = ga_min, ga_max = ga_max, mutp = mutp, popsize = popsize, cc=1.)
				
				tscore = np.loadtxt(DIR+'Workspace/Interp/'+'comb_score0.txt').T#pd.read_csv(DIR+'Workspace/Interp/'+'comb_score0.txt',sep='\s+', header=None)
				score = tscore[0]
				
				count00 = count00 + 1
				print('round', count00, 'of', i_var-1,'red_chi2 =', score)
		...
		sp = np.loadtxt(DIR+'Workspace/Interp/Best_comb_selected.csv', dtype=str,delimiter=',',usecols=[matrix], skiprows=1).T
		...
		spn = np.loadtxt(DIR+'Workspace/Interp/Best_comb_selected.csv', dtype=str,delimiter=',',usecols=[matrix], skiprows=1).T
		...
		for index0 in range(i_var):
			...
			if listmut[index0] in list2:
				...
				f = gam.gamod(new_tau,new_etau, home1,home2, Stats=St, freq_stat = freqs, gen=gen, ga_min = ga_min, ga_max = ga_max, mutp = mutp, popsize =popsize, cc=count0)
				
				tscore = np.loadtxt(DIR+'Workspace/Interp2/'+'comb_score0.txt').T#pd.read_csv(DIR+'Workspace/Interp/'+'comb_score0.txt',sep='\s+', header=None)
				score = tscore[0]
				count0 = count0 + 1
				print('round', count0, 'of', countl,'red_chi2 =', score)
		...
		sp = np.loadtxt(DIR+'Workspace/Interp2/Best_comb_selected.csv', dtype=str,delimiter=',',usecols=[matrix], skiprows=1).T
		...
		if group_comb <= q:
			...
			def func2(all):
				...
				for k in range(len(all)):
					...
					f = gam.gamod(new_tau,new_etau, home1, home2, Stats=St, freq_stat = freqs, gen=gen, ga_min = ga_min, ga_max = ga_max, mutp = mutp, popsize = popsize, cc=count)
					
					sh.rm(sh.glob(DIR+'Workspace/Processing/Interp_proc/*.dat'))
					sh.rm(sh.glob(DIR+'Workspace/Processing/Interp_proc/OutFile.txt'))
					
					
					tscore = np.loadtxt(DIR+'Workspace/Processing/Interp_proc/'+'comb_score0.txt').T#pd.read_csv(DIR+'Workspace/Interp/'+'comb_score0.txt',sep='\s+', header=None)
					score = tscore[0]
					count = count + 1
					print('round', count, 'of', len(all),'red_chi2 =', score)
			...
			print(' ')
			print(' ')
			print('ENIIGMA info: ERROR: Cannot combine' +' ' +str(q)+ ' ' + 'files in groups of' + ' ' +str(group_comb)+'!')
